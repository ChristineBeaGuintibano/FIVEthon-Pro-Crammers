...
quit_button.pack(side=tk.LEFT)from tkinter import *
import time
...
class StopWatch(Frame):  
    ...
    def _setLapTime(self, elap):
        ...
        seconds = int(elap - minutes*60.0)
        hseconds = int((elap - minutes*60.0 - seconds)*100)            
        T1 = f'Time #{num}: ' + '%02d:%02d:%02d' % (minutes, seconds, hseconds)
        return T1
        
    def _setLapTime2(self, elap):
        minutes = int(elap/60)
        seconds = int(elap - minutes*60.0)
        hseconds = int((elap - minutes*60.0 - seconds)*100)            
        T2 = 'Interval: + %02d:%02d:%02d' % (minutes, seconds, hseconds)
        return T2            
    ...
    def Lap(self):
        ...
        tempo2 = self._elapsedtime - self.lapmod2
        if self._running:
            self.laps.append(self._setLapTime(tempo))
            self.m.insert(END, self.laps[-1])
            self.m.yview_moveto(1)
            self.laps.append(self._setLapTime2(tempo2))
            self.m.insert(END, self.laps[-1])
            self.m.yview_moveto(1)
            self.lapmod2 = self._elapsedtime
